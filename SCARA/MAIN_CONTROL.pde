void controlGO() {
  ...
  { 
    ...
    {  
      ...
      hideMode3B();
      hideMode3C();
      ellipse(m3s1_posX, m3s1_posY, 15, 15);

      value_posX=m3s1_posX_;
      value_posY=m3s1_posY_;
      ...
      value_akurasi_posX=float(abs(value_posX-value_feedback_shoulder))/value_shoulder*100;
      value_akurasi_posY=float(abs(value_posY-value_feedback_elbow))/value_elbow*100;
      value_akurasi=100-(value_akurasi_posX+value_akurasi_posY)/2;
      if (value_akurasi<0) {
        value_akurasi=0;
      }
      ...
      fill(c_putih);
      textFont(f18);
      text(value_akurasi+"%", 1179, CO-143); //Akurasi
      textFont(f16);
    }
